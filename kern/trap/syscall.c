...
void sys_free_user_mem(uint32 virtual_address, uint32 size) {
	if (virtual_address >= USER_HEAP_START && virtual_address <= USER_HEAP_MAX) {
		if (size != 0 && virtual_address + size < USER_HEAP_MAX) {
...
void sys_enqueue(struct Env_Queue* queue, struct semaphore* sem) {
	//cprintf(" stfu \n");
	// struct Env* mernfrfr = get_cpu_proc();
	struct Env* mernfrfr = cur_env;
	//cprintf(" stfu 2\n");
	acquire_spinlock(&ProcessQueues.qlock);
	//cprintf(" stfu3 \n");
	enqueue(queue, mernfrfr);
	mernfrfr->env_status = ENV_BLOCKED;
	sem->semdata->lock = 0;
	//cprintf(" stfu 4\n");
	//popcli();
	sched();
...
struct Env* sys_get_current_env(void) {
...
void sys_env_set_priority(int32 envID, int priority) {
	//TODO: [PROJECT'24.MS3 - #06] [3] SYSTEM CALLS - Add suitable code here
