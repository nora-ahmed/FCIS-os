...
//			int32 curEnv = sys_getenvid();
			init_queue(&(ProcessQueues.env_ready_queues[i]));
//			env_set_priority(curEnv,i);
//			env_set_priority(curEnv->env_id, i);
...
struct Env* fos_scheduler_PRIRR()
{
	...
	//Comment the following line
//	panic("Not implemented yet");
//	 dont forget to set the quntum in the end

	struct Env* current_pro=get_cpu_proc();


//	struct Env_Queue *process_in_queue=env_ready_queues;
	if(current_pro!=NULL)
	{
//		enqueue the process out of the cpu and back to its right priority queue
		sched_insert_ready(current_pro);


//	looking for the queue with highest praiorty

	for(uint8 priorty=0;priorty<num_of_ready_queues;priorty++){
//		i want to go through the array of priority queues where
//		 i want to access the queue of the highest priorty to return (dequue )
//		the next process in the cpu
		struct Env_Queue *rua =&ProcessQueues.env_ready_queues[priorty];
		if(&rua->size>0)
		{

			current_pro=dequeue(rua);
			kclock_set_quantum(quantums[0]);
						return current_pro;
...
//	thats if no process in any queue,thus no processs is gonna be in cpu
	return NULL;
}
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR()) {
				...
				//panic("Not implemented yet");
				struct Env* mypro;
				struct Env_Queue* ready_processes = ProcessQueues.env_ready_queues;

				for (int i = 0; i < num_of_ready_queues; i++) {
					int j = 0;
					mypro = ready_processes[i].lh_first;
					do {
						mypro->wait_ticks = timer_ticks() - mypro->nClocks;

						if (mypro->wait_ticks >= STARVTHRESH) {

	//						switch (mypro->priority) {
	//						case 5:
	//							mypro->priority = 4;
	//							break;
	//						case 4:
	...
	//						}
						...
						j++;
					} while (j <= ready_processes[i].size);
				}
			}
