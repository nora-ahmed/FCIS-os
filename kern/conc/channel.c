...
void sleep(struct Channel *chan, struct spinlock* lk)
{
	...
	//Your Code is Here...
	release_spinlock(lk);
	    struct Env* process_to_sleep = get_cpu_proc();
	    process_to_sleep->env_status = ENV_BLOCKED;
	    acquire_spinlock(&ProcessQueues.qlock);
	    enqueue(&chan->queue, process_to_sleep);
	    while(process_to_sleep->env_status==ENV_BLOCKED){
	    sched();
	    }
	    acquire_spinlock(lk);
...
void wakeup_all(struct Channel *chan)
{
	...
	//Your Code is Here...
	    acquire_spinlock(&ProcessQueues.qlock);
	    struct Env* process;
	    while ((process = dequeue(&chan->queue)) != NULL){
