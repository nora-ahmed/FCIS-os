...
//	env_page_ws_invalidate(e, virtual_address);
		...
		struct WorkingSetElement* workingsetelementyah = (struct WorkingSetElement*)kmalloc(sizeof(struct WorkingSetElement));
	//	pt_set_page_permissions(e->env_page_directory,(uint32)workingsetelementyah,PERM_AVAILABLE,0);
	struct FrameInfo *fram;
		int allo = allocate_frame(&fram);
		if (allo == 0) {
		    map_frame(e->env_page_directory, fram, virtual_address, PERM_WRITEABLE | PERM_USER);
		} else {
		    panic("AAAAAAA");
		}
		workingsetelementyah->virtual_address = virtual_address;
		if (workingsetelementyah == NULL) {
		...
		//LIST_INSERT_TAIL(&(e->page_WS_list), workingsetelementyah);

		return workingsetelementyah;
}
