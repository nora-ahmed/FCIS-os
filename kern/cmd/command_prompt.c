...
int process_command(int number_of_arguments, char** arguments)
{
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
		{
			if((strcmp(arguments[0], commands[i].name) == 0)&& (commands[i].num_of_args == -1))
			{
				if(number_of_arguments > 1)
				{
					return i;
		   ...
		   else if ((strcmp(arguments[0], commands[i].name) == 0) && ((number_of_arguments-1)!= commands[i].num_of_args) )
			{
				struct Command * it = NULL;
				LIST_FOREACH (it, &foundCommands)
				{
		...
		for(int j = 0; j < NUM_OF_COMMANDS; j++)
		{
			int x = 0;
			int y = 0;
			while(commands[j].name[y] != '\0')
			{
				if(arguments[0][x] != commands[j].name[y])
				{
					y++;
				}
				else
