...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	uint32* ppp=(uint32*)kmalloc(KERNEL_STACK_SIZE);
	...
	if(ppp == NULL)
			...
			pt_set_page_permissions(ptr_user_page_directory,(uint32)ppp,0,PERM_PRESENT);

			//ppp = (uint32*)((uint32)ppp & ~PERM_PRESENT);
...
////e->nour=1;
//	for(uint32 i= (uint32)e->USER_HARD_LIMIT+(4*kilo);i<(uint32)USER_HEAP_MAX;i+=(4*kilo)){
//
//		user_Pages[((uint32)i-((uint32)e->USER_HARD_LIMIT+(4*kilo)))/(4*kilo)].numOfPages=((uint32)USER_HEAP_MAX-((uint32)e->USER_HARD_LIMIT+(4*kilo)))/(4*kilo);
//
//		user_Pages[((uint32)i-((uint32)e->USER_HARD_LIMIT+(4*kilo)))/(4*kilo)].va=(uint32)e->USER_HARD_LIMIT+(4*kilo);
//		user_Pages[((uint32)i-((uint32)e->USER_HARD_LIMIT+(4*kilo)))/(4*kilo)].is_f=1;
//		user_Pages[((uint32)i-((uint32)e->USER_HARD_LIMIT+(4*kilo)))/(4*kilo)].is_m=0;
//	}
